import{_ as s,c as n,o as a,a as l}from"./app.04d6123c.js";const d=JSON.parse('{"title":"如何为非 GKI 内核集成 KernelSU","description":"","frontmatter":{},"headers":[{"level":2,"title":"使用 kprobe 集成","slug":"使用-kprobe-集成","link":"#使用-kprobe-集成","children":[]},{"level":2,"title":"Manully modify the kernel source","slug":"manully-modify-the-kernel-source","link":"#manully-modify-the-kernel-source","children":[]}],"relativePath":"zh_CN/guide/how-to-integrate-for-non-gki.md"}'),e={name:"zh_CN/guide/how-to-integrate-for-non-gki.md"},p=l(`<h1 id="如何为非-gki-内核集成-kernelsu" tabindex="-1">如何为非 GKI 内核集成 KernelSU <a class="header-anchor" href="#如何为非-gki-内核集成-kernelsu" aria-hidden="true">#</a></h1><p>KernelSU 可以被集成到非 GKI 内核中，现在它最低支持到内核 4.14 版本；理论上也可以支持更低的版本。</p><p>由于非 GKI 内核的碎片化极其严重，因此通常没有统一的方法来编译它，所以我们也无法为非 GKI 设备提供 boot 镜像。但你完全可以自己集成 KernelSU 然后编译内核使用。</p><p>首先，你必须有能力从你设备的内核源码编译出一个可以开机并且能正常使用的内核，如果内核不开源，这通常难以做到。</p><p>如果你已经做好了上述准备，那有两个方法来集成 KernelSU 到你的内核之中。</p><ol><li>借助 <code>kprobe</code> 自动集成</li><li>手动</li></ol><h2 id="使用-kprobe-集成" tabindex="-1">使用 kprobe 集成 <a class="header-anchor" href="#使用-kprobe-集成" aria-hidden="true">#</a></h2><p>KernelSU 使用 kprobe 机制来做内核的相关 hook，如果 <em>kprobe</em> 可以在你编译的内核中正常运行，那么推荐用这个方法来集成。</p><p>首先，把 KernelSU 添加到你的内核源码树，在内核的根目录执行以下命令：</p><div class="language-sh"><button title="Copy Code" class="copy"></button><span class="lang">sh</span><pre class="shiki material-palenight"><code><span class="line"><span style="color:#FFCB6B;">curl</span><span style="color:#A6ACCD;"> </span><span style="color:#C3E88D;">-LSs</span><span style="color:#A6ACCD;"> </span><span style="color:#89DDFF;">&quot;</span><span style="color:#C3E88D;">https://raw.githubusercontent.com/tiann/KernelSU/main/kernel/setup.sh</span><span style="color:#89DDFF;">&quot;</span><span style="color:#A6ACCD;"> </span><span style="color:#89DDFF;">|</span><span style="color:#A6ACCD;"> </span><span style="color:#FFCB6B;">bash</span><span style="color:#A6ACCD;"> </span><span style="color:#C3E88D;">-</span></span>
<span class="line"></span></code></pre></div><p>然后，你需要检查你的内核是否开启了 <em>kprobe</em> 相关的配置，如果没有开启，需要添加以下配置：</p><div class="language-txt"><button title="Copy Code" class="copy"></button><span class="lang">txt</span><pre class="shiki material-palenight"><code><span class="line"><span style="color:#A6ACCD;">CONFIG_KPROBES=y</span></span>
...
<span class="line"><span style="color:#A6ACCD;"></span></span></code></pre></div><p>最后，重新编译你的内核即可。</p><p>如果你在集成 KernelSU 之后手机无法启动，那么很可能你的内核中 <strong>kprobe 工作不正常</strong>，你需要修复这个 bug 或者用第二种方法。</p><h2 id="manully-modify-the-kernel-source" tabindex="-1">Manully modify the kernel source <a class="header-anchor" href="#manully-modify-the-kernel-source" aria-hidden="true">#</a></h2><p>如果 kprobe 工作不正常（通常是上游的 bug 或者内核版本过低），那你可以尝试这种方法：</p><p>首先，把 KernelSU 添加到你的内核源码树，在内核的根目录执行以下命令：</p><div class="language-sh"><button title="Copy Code" class="copy"></button><span class="lang">sh</span><pre class="shiki material-palenight"><code><span class="line"><span style="color:#FFCB6B;">curl</span><span style="color:#A6ACCD;"> </span><span style="color:#C3E88D;">-LSs</span><span style="color:#A6ACCD;"> </span><span style="color:#89DDFF;">&quot;</span><span style="color:#C3E88D;">https://raw.githubusercontent.com/tiann/KernelSU/main/kernel/setup.sh</span><span style="color:#89DDFF;">&quot;</span><span style="color:#A6ACCD;"> </span><span style="color:#89DDFF;">|</span><span style="color:#A6ACCD;"> </span><span style="color:#FFCB6B;">bash</span><span style="color:#A6ACCD;"> </span><span style="color:#C3E88D;">-</span></span>
...
<span class="line"></span></code></pre></div><p>主要是要改四个地方：</p><ol><li>do_faccessat, usually in <code>fs/open.c</code></li><li>do_execveat_common, usually in <code>fs/exec.</code></li><li>vfs_read, usually in <code>fs/read_write.c</code></li><li>vfs_statx, usually in <code>fs/stat.c</code></li></ol><p>改完之后，打开 <code>KernelSU/kernel/ksu.c</code>，注释 <code>enable_sucompat()</code>调用，然后重新编译内核即可。</p>`,23),o=[p];function t(c,r,i,C,A,D){return a(),n("div",null,o)}const f=s(e,[["render",t]]);export{d as __pageData,f as default};
