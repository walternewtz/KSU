import{_ as s,c as n,o as a,a as e}from"./app.1f9a666d.js";const d=JSON.parse('{"title":"Làm thế nào để tích hợp KernelSU vào thiết bị không sử dụng GKI ?","description":"","frontmatter":{},"headers":[{"level":2,"title":"Tích hợp vào kprobe","slug":"tich-hop-vao-kprobe","link":"#tich-hop-vao-kprobe","children":[]},{"level":2,"title":"Chỉnh sửa mã nguồn kernel thủ công","slug":"chinh-sua-ma-nguon-kernel-thu-cong","link":"#chinh-sua-ma-nguon-kernel-thu-cong","children":[]}],"relativePath":"vi_VN/guide/how-to-integrate-for-non-gki.md"}'),l={name:"vi_VN/guide/how-to-integrate-for-non-gki.md"},p=e(`<h1 id="lam-the-nao-đe-tich-hop-kernelsu-vao-thiet-bi-khong-su-dung-gki" tabindex="-1">Làm thế nào để tích hợp KernelSU vào thiết bị không sử dụng GKI ? <a class="header-anchor" href="#lam-the-nao-đe-tich-hop-kernelsu-vao-thiet-bi-khong-su-dung-gki" aria-hidden="true">#</a></h1><p>KernelSU có thể được tích hợp vào kernel không phải GKI và hiện tại nó đã được backport xuống 4.14, thậm chí cũng có thể chạy trên kernel thấp hơn 4.14.</p><p>Do các kernel không phải GKI bị phân mảnh nên chúng tôi không có cách build thống nhất, vì vậy chúng tôi không thể cung cấp các boot image không phải GKI. Nhưng bạn có thể tự build kernel với KernelSU được tích hợp vào.</p><p>Đầu tiên, bạn phải build được kernel từ nguồn có khả năng boot được , nếu kernel không có mã nguồn mở thì rất khó để chạy KernelSU cho thiết bị của bạn.</p><p>Nếu bạn có thể build kernel khởi động được, có hai cách để tích hợp KernelSU vào mã nguồn kernel:</p><ol><li>Tự động với <code>kprobe</code></li><li>Thủ công</li></ol><h2 id="tich-hop-vao-kprobe" tabindex="-1">Tích hợp vào kprobe <a class="header-anchor" href="#tich-hop-vao-kprobe" aria-hidden="true">#</a></h2><p>KernelSU sử dụng kprobe để thực hiện hook kernel, nếu <em>kprobe</em> chạy tốt trong kernel của bạn thì nên sử dụng cách này.</p><p>Đầu tiên, thêm KernelSU vào mã nguồn kernel của bạn:</p><div class="language-sh"><button title="Copy Code" class="copy"></button><span class="lang">sh</span><pre class="shiki material-palenight"><code><span class="line"><span style="color:#FFCB6B;">curl</span><span style="color:#A6ACCD;"> </span><span style="color:#C3E88D;">-LSs</span><span style="color:#A6ACCD;"> </span><span style="color:#89DDFF;">&quot;</span><span style="color:#C3E88D;">https://raw.githubusercontent.com/tiann/KernelSU/main/kernel/setup.sh</span><span style="color:#89DDFF;">&quot;</span><span style="color:#A6ACCD;"> </span><span style="color:#89DDFF;">|</span><span style="color:#A6ACCD;"> </span><span style="color:#FFCB6B;">bash</span><span style="color:#A6ACCD;"> </span><span style="color:#C3E88D;">-</span></span>
<span class="line"></span></code></pre></div><p>Sau đó, bạn nên kiểm tra xem <em>kprobe</em> có được bật trong config của bạn hay không, nếu không, vui lòng thêm các cấu hình sau vào:</p><div class="language-"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki material-palenight"><code><span class="line"><span style="color:#A6ACCD;">CONFIG_KPROBES=y</span></span>
...
<span class="line"><span style="color:#A6ACCD;"></span></span></code></pre></div><p>Rồi build lại kernel của bạn, KernelSU sẽ hoạt động ok.</p><p>Nếu bạn thấy rằng KPROBES vẫn chưa được kích hoạt, bạn có thể thử bật <code>CONFIG_MODULES</code>. (Nếu nó vẫn không có hiệu lực, hãy sử dụng <code>make menuconfig</code> để tìm kiếm các phần phụ thuộc khác của KPROBES)</p><p>Nhưng nếu bạn gặp bootloop khi tích hợp KernelSU thì có thể <em>kprobe bị hỏng trong kernel</em>, bạn nên fix lỗi kprobe trong mã nguồn hoặc dùng cách 2.</p><h2 id="chinh-sua-ma-nguon-kernel-thu-cong" tabindex="-1">Chỉnh sửa mã nguồn kernel thủ công <a class="header-anchor" href="#chinh-sua-ma-nguon-kernel-thu-cong" aria-hidden="true">#</a></h2><p>Nếu kprobe không thể hoạt động trong kernel của bạn (có thể là lỗi do upstream hoặc kernel dưới bản 4.8), thì bạn có thể thử cách này:</p><p>Đầu tiên, thêm KernelSU vào mã nguồn kernel của bạn:</p><div class="language-sh"><button title="Copy Code" class="copy"></button><span class="lang">sh</span><pre class="shiki material-palenight"><code><span class="line"><span style="color:#FFCB6B;">curl</span><span style="color:#A6ACCD;"> </span><span style="color:#C3E88D;">-LSs</span><span style="color:#A6ACCD;"> </span><span style="color:#89DDFF;">&quot;</span><span style="color:#C3E88D;">https://raw.githubusercontent.com/tiann/KernelSU/main/kernel/setup.sh</span><span style="color:#89DDFF;">&quot;</span><span style="color:#A6ACCD;"> </span><span style="color:#89DDFF;">|</span><span style="color:#A6ACCD;"> </span><span style="color:#FFCB6B;">bash</span><span style="color:#A6ACCD;"> </span><span style="color:#C3E88D;">-</span></span>
<span class="line"></span></code></pre></div><p>Sau đó, thêm lệnh gọi KernelSU vào mã nguồn kernel, đây là một patch bạn có thể tham khảo:</p><div class="language-diff"><button title="Copy Code" class="copy"></button><span class="lang">diff</span><pre class="shiki material-palenight has-diff"><code><span class="line"><span style="color:#A6ACCD;">diff --git a/fs/exec.c b/fs/exec.c</span></span>
